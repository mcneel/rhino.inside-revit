...
namespace RhinoInside.Revit.GH.Components.Annotations
{
  ...
  public class AddDiameterDimension : ElementTrackerComponent
  {
    public override Guid ComponentGuid => new Guid("00D46C47-E8AE-4EC2-98F8-77F2542C26F9");
    public override GH_Exposure Exposure => SDKCompliancy(GH_Exposure.primary);
    protected override string IconTag => string.Empty;

    public AddDiameterDimension() : base
    (
      name: "Add Diameter Dimension",
      nickname: "D-Dim",
      description: "Given an arc referece, it adds a diameter dimension to the given View",
      category: "Revit",
    ...
    protected override void TrySolveInstance(IGH_DataAccess DA)
    {
      ...
      (
        view.Document, _Output_, dimension =>
        {
          ...
          if (!Parameters.ElementType.GetDataOrDefault(this, DA, "Type", out ARDB.DimensionType type, Types.Document.FromValue(view.Document), ARDB.ElementTypeGroup.DiameterDimensionType)) return null;
    ...
    ARDB.RadialDimension Reconstruct(ARDB.RadialDimension dimension, ARDB.View view, IList<ARDB.Reference> references, ARDB.DimensionType type)
    {
      ...
      {
        dimension = ARDB.RadialDimension.Create(view.Document, view, references.FirstOrDefault(), isDiameter: true);
        if (type is object) dimension.ChangeTypeId(type.Id);
      }

  ...
  public class FreeFormByCurves : ReconstructElementComponent
  {
    ...
    )
    {
      ...
      if (profiles.Count == 1)
      {
        ...
        using (var sketchPlane = DB.SketchPlane.Create(doc, plane.ToHost()))
      ...
      else
      {
        using (var referenceArrayArray = new DB.ReferenceArrayArray())
        {
          ...
          foreach (var profile in profiles)
          {
            using (var sketchPlane = DB.SketchPlane.Create(doc, planes[index++].ToHost()))
            {

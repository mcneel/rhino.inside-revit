...
{
  public class Category : ElementIdNonGeometryParam<Types.Documents.Categories.Category, DB.Category>
  {
    ...
    protected override Types.Documents.Categories.Category PreferredCast(object data) => Types.Documents.Categories.Category.FromValue(data);
    ...
    public override void AppendAdditionalMenuItems(ToolStripDropDown menu)
    {
...

      {
        ...
        (
          ...
          PersistentData.get_FirstItem(true) is Types.Documents.Categories.Category firstValue &&
    ...
    private void RefreshCategoryList(ListBox listBox, DB.CategoryType categoryType)
    {
      ...
      try
      {
        ...
        var current = default(Types.Documents.Categories.Category);
        if (SourceCount == 0 && PersistentDataCount == 1)
        {
          if (PersistentData.get_FirstItem(true) is Types.Documents.Categories.Category firstValue)
            current = firstValue as Types.Documents.Categories.Category;
        }
      ...
      if (sender is ListBox listBox)
      {
        if (listBox.SelectedIndex != -1)
        {
          if (listBox.Items[listBox.SelectedIndex] is IGH_GooProxy value)
          {
            ...
            PersistentData.Append(value.ProxyOwner as Types.Documents.Categories.Category);
          }
        }

...
namespace RhinoInside.Revit.GH.Parameters.Elements.Material
{
  public class Material : ElementIdNonGeometryParam<Types.Elements.Material.Material, DB.Material>
  {
    public override GH_Exposure Exposure => GH_Exposure.tertiary;
    public override Guid ComponentGuid => new Guid("B18EF2CC-2E67-4A5E-9241-9010FB7D27CE");
    protected override Types.Elements.Material.Material PreferredCast(object data) => Types.Elements.Material.Material.FromElement(data as DB.Material) as Types.Elements.Material.Material;
    ...
    private void RefreshMaterialsList(ListBox listBox, string materialClass)
    {
      ...
      try
      {
        ...
        var current = default(Types.Elements.Material.Material);
        if (SourceCount == 0 && PersistentDataCount == 1)
        {
          if (PersistentData.get_FirstItem(true) is Types.Elements.Material.Material firstValue)
            current = firstValue.Duplicate() as Types.Elements.Material.Material;
        }
        ...
        using (var collector = new DB.FilteredElementCollector(doc).OfClass(typeof(DB.Material)))
        {
          ...
          {
            var tag = new Types.Elements.Material.Material(material);
      ...
      if (sender is ListBox listBox)
      {
        if (listBox.SelectedIndex != -1)
        {
          if (listBox.Items[listBox.SelectedIndex] is IGH_GooProxy value)
          {
            ...
            PersistentData.Append(value.ProxyOwner.Duplicate() as Types.Elements.Material.Material);
          }
        }

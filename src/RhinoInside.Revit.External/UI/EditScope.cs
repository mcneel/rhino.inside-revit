...
namespace RhinoInside.Revit.External.UI
{
  internal sealed class EditScope : IDisposable
  {
    ...
    public EditScope(UIHostApplication app)
    {
      ...
      if (WasExposed) HostedApplication.Active.MainWindow.Visible = false;

      ExternalApplication.HostMainWindow.Enabled = true;
    ...
    {
      if (WasExposed) HostedApplication.Active.MainWindow.Visible = WasExposed;
      HostedApplication.Active.MainWindow.ShowOwnedPopups();

      ExternalApplication.HostMainWindow.Enabled = WasEnabled;
